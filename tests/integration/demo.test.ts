...
describe("separate Threadkeeper demo dataset", () => {
  ...
  test("demo status has the deterministic Northstar counts", async () => {
  ...
  test("demo routes expose evidence and completed archaeology", async () => {
    ...
    expect(loops.items[0].id).toBe(DEMO_IDS.landingLoop);
    expect(loops.items[0].dueAt).toBe(Date.parse("2026-08-20T00:00:00Z"));
    expect(loops.items[0].sources[0].excerpt).toContain("landing page is still unpublished");
    ...
    expect((await response.json()).items[0].title).toBe("Northstar product launch");
  });
  ...
  test("demo actions mutate only demo, and reset restores the seed", async () => {
    ...
    runDemoAction({ action: "reject_proposal", targetId: DEMO_IDS.cadenceProposal, reason: "test" });
  ...
  test("accepting cadence makes monthly current, preserves weekly history, and is idempotent", async () => {
    const demo = resetDemoDbForTests(path("cadence.sqlite"));
    await getDemoStatus();
    const before = demo.query(`SELECT id, object_json, status FROM claims WHERE subject='Northstar' AND predicate='publishing cadence'`).all() as Array<{ id: string; object_json: string; status: string }>;
    expect(before).toHaveLength(1);
    expect(JSON.parse(before[0]!.object_json)).toBe("weekly");
    expect(before[0]!.status).toBe("active");

    const accepted = await runDemoAction({ action: "accept_proposal", targetId: DEMO_IDS.cadenceProposal });
    expect(accepted.result).toMatchObject({ supersededClaimId: DEMO_IDS.weeklyCadenceClaim });
    expect(accepted.result.createdClaimId).toBeTruthy();
    expect("updatedClaimId" in accepted.result).toBe(false);

    const claims = demo.query(`SELECT id, object_json, status, supersedes_claim_id FROM claims WHERE subject='Northstar' AND predicate='publishing cadence' ORDER BY created_at`).all() as Array<{ id: string; object_json: string; status: string; supersedes_claim_id: string | null }>;
    expect(claims).toHaveLength(2);
    expect(claims.map((claim) => [JSON.parse(claim.object_json), claim.status])).toEqual([["weekly", "superseded"], ["monthly", "active"]]);
    expect(claims[1]!.supersedes_claim_id).toBe(DEMO_IDS.weeklyCadenceClaim);
    const threads = await listDemoThreads({ status: "all", limit: 20 });
    expect(threads.items.find((item: any) => item.id === DEMO_IDS.northstar)?.cadence).toEqual({ current: "monthly", history: ["weekly"] });

    const repeated = await runDemoAction({ action: "accept_proposal", targetId: DEMO_IDS.cadenceProposal });
    ...
    const restored = demo.query(`SELECT object_json, status FROM claims WHERE subject='Northstar' AND predicate='publishing cadence'`).all() as Array<{ object_json: string; status: string }>;
    expect(restored).toHaveLength(1);
    expect([JSON.parse(restored[0]!.object_json), restored[0]!.status]).toEqual(["weekly", "active"]);
    expect((await getDemoStatus()).proposalCount).toBe(1);
  });

  test("demo action endpoint rejects unsupported actions without falling through", async () => {
    const response = await actionRoute.POST(new Request("http://local/action", { method: "POST", body: JSON.stringify({ dataset: "demo", action: "archive_thread", targetId: DEMO_IDS.northstar }) }));
