...
function cadence(handle: Database) {
  const rows = handle.query(`SELECT id, object_json, status, supersedes_claim_id supersedesClaimId FROM claims WHERE subject='Marlowe billing review call' AND predicate='call time' ORDER BY created_at`).all() as Record<string, any>[];
...
function thread(handle: Database, id: string) {
  ...
  return row && id === DEMO_IDS.billing ? { ...row, cadence: cadence(handle) } : row;
}
...
function shapeProposal(handle: Database, row: Record<string, any>) {
  ...
  return { id: row.id, kind: "proposal", proposalType: row.proposal_type, targetType: row.target_type, targetId: row.target_id, operation: row.operation, payload, title: "Move the Marlowe billing review call: Wednesday → Thursday", description: "Accepting creates an active Thursday 10:00 AM call-time claim and supersedes the active Wednesday 2:00 PM claim.", impact: "Makes Thursday 10:00 AM current while preserving Wednesday 2:00 PM as superseded historical context.", status: row.status, confidence: scoreConfidence(confidence), confidenceComponents: confidence, risk: confidence.risk, createdAt: row.created_at, sources, excerpt: sources[0]?.excerpt ?? null };
}
function shapeConnection(handle: Database, row: Record<string, any>) {
  const sources = sourcesFor(handle, DEMO_IDS.bridgeArtifact); const from = thread(handle, row.from_id); const to = thread(handle, row.to_id); const score = parse(row.score_json);
  return { id: row.id, kind: "connection", fromType: row.from_type, fromId: row.from_id, toType: row.to_type, toId: row.to_id, relationType: row.relation_type, title: "Billing fix may unblock the overdue Marlowe invoice", description: row.explanation, summary: `${from?.title ?? "Marlowe invoice follow-up"} → ${to?.title ?? "Marlowe billing fix"}`, explanation: row.explanation, score, confidence: score.final, status: row.status, createdAt: row.created_at, sources };
}
